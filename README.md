...
- 默认 `vscode` Dark+ 主题（可持久切换，主题选择器也提供内置 `default`），`<Space>` 作为 leader，`\` 作为 localleader。
...
Java / Rust 也可用 `:FastLspStart jdtls` / `:FastLspStart rust_analyzer`，不加载
完整集成与调试插件。Java 仍需可用的 JDK 21+；Rust 默认关闭保存时检查、构建脚本和过程宏。
...
缺少 fzf 或 rg 时，搜索入口改用原生方式：`vim.ui.input` 的文件名补全打开文件，
分批读取项目文件，用 Vim 正则把匹配送进 quickfix；遵守目录中的 `.gitignore` 和 `.ignore`，
跳过版本库内部目录、符号链接、二进制及大于 1.5 MiB 的文件，最多检查 50,000 个目录项、
收集 10,000 个匹配行，结果受限时会提示。缓冲区和最近文件用原生选择列表；
...
- `<leader>w` / `<leader>W` ：保存当前文件 / 保存全部已修改文件（`:wall`）
- `<leader>R` ：原地重启，恢复文件、分屏和光标位置；未保存修改或运行中的终端任务会先确认，可取消。普通和 fast 模式均支持。
...
- `<leader>uT` / `:TransparentToggle` ：切换并持久保存透明背景，切换主题后仍生效。编辑区、行号栏、文件树、sticky context、buffer 标签栏及有边框弹窗透明，无边框菜单和编辑区选中行保留底色；透明程度由终端设置决定。透明模式下，仅 TokyoNight（fg_gutter）和 Catppuccin（surface1）的窗口分隔线改用主题色板中的原生暗灰色，其他主题保留原色；buffer 标签栏（含 Explorer 标题）底部用连续细线分隔，不额外占一行。切换时刷新当前主题及插件颜色缓存，关闭后恢复主题原色。状态保存在 `stdpath("state")/transparent.txt`，默认关闭。
...
- 统一样式下，无边框小弹窗保留菜单底色；有边框浮窗在非透明背景下使用当前主题原本的弹窗底色（NormalFloat），透明背景下不设置底色。通知的正文、空行、标题留白和行尾一起切换，避免只有文字带背景；淡入淡出及内容更新保持一致，通知修复不受弹窗样式开关影响。
