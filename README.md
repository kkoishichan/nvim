...
分屏分别绘制可见区域。键盘分页和 `zt` / `zz` / `zb` 使用原生响应，取消逐行播放的动画；
`Ctrl-E` / `Ctrl-Y` 恢复原生单行滚动和计数行为。原来的 Tree-sitter 高亮及配色保持不变。
验证方法、测量结果及适用范围见[滚动性能验证](docs/scrolling.md)、
[后续性能优化](docs/performance-followup.md)及[键盘滚动验证](docs/navigation-performance.md)。
...
- `python3 scripts/benchmark-navigation.py --file /path/to/code.py --baseline /path/to/old-checkout`
  对比半页、整页按键到最终视口刷新的耗时，包含完整高亮和已配置的滚动动画；不包含终端和设备延迟。
