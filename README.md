...
- `<leader>uT` / `:TransparentToggle` ：切换并持久保存透明背景，切换主题后仍生效。编辑区、行号栏、文件树、sticky context、buffer 标签栏及有边框弹窗透明，无边框菜单和编辑区选中行保留底色；透明程度由终端设置决定。透明模式下，窗口分隔线使用从主题配色中提亮的灰色；buffer 标签栏（含 Explorer 标题）底部用连续细线分隔，不额外占一行。切换时刷新当前主题及插件颜色缓存，关闭后恢复主题原色。状态保存在 `stdpath("state")/transparent.txt`，默认关闭。
