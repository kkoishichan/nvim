-- Conform, the formatter definitions both modes share. The tool table is the
-- same everywhere; only the automatic save hooks differ, so a mode without
-- format-on-save still has <leader>cf and :ConformInfo with identical results.
...
return function()
	local on_save = mode.capabilities().format_on_save
	return {
		{
			"stevearc/conform.nvim",
			-- Only a save can pull the formatter in, and only when this mode
			-- formats on save; otherwise the keys and :ConformInfo load it.
			event = on_save and { "BufWritePre" } or nil,
			cmd = { "ConformInfo" },
			keys = {
				{
					"<leader>cf",
			...
			opts = {
				...
				format_on_save = on_save and function(bufnr)
				...
				format_after_save = on_save and function(bufnr)
					local policy = require("user.core.format_policy")
