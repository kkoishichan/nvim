...
function M.setup(context)
	...
	-- :PdfOpen -> the system viewer). Adjacent pages are prefetched into the PNG cache
	...
	local pdf_cache_dir = vim.fs.joinpath(vim.fn.stdpath("cache"), "user", "pdf-preview")
	local pdf_cache_max_age = 30 * 24 * 60 * 60
	local pdf_cache_max_bytes = 512 * 1024 * 1024
	...
	local function prune_pdf_cache()
		...
		local handle = vim.uv.fs_scandir(pdf_cache_dir)
		if not handle then
			return
		end
			...
			if kind == "file" and name:match("%.png$") then
				...
				if stat and not pending_pdf_conversions[path] then
					...
					if modified > 0 and now - modified > pdf_cache_max_age then
						...
						table.insert(files, { path = path, modified = modified, size = stat.size or 0 })
					end
				end
			end
		end
		...
		for _, file in ipairs(files) do
			retained = retained + file.size
			if retained > pdf_cache_max_bytes then
	...
	local function pdf_cache_file(file, page, zoom)
		...
		vim.fn.mkdir(pdf_cache_dir, "p", 448) -- 0700: rendered pages may contain sensitive material.
		pcall(vim.uv.fs_chmod, pdf_cache_dir, 448)
	...
	local function convert_pdf_preview(file, page, zoom, png, callback, opts)
		...
		if vim.uv.fs_stat(png) then
			local now = os.time()
			pcall(vim.uv.fs_utime, png, now, now)
			callback(png)
			return
		end
	...
	local function render_pdf_preview(bufnr, opts)
		...
		convert_pdf_preview(file, page, zoom, png, function(png_file)
			...
			if not ok then
				vim.notify(
					"image.nvim is not available for PDF preview",
