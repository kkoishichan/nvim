...
add_group() {
	local existing
	case "$1" in
	static | integration | performance | ai | languages | projects | project_actions | toolchain | terminals | workflow_actions | signature | lifecycle | pdf_lifecycle | windows | layout_session | ui_themes | offline_assets | deployment | workflow_python_js | workflow_native | workflow_java_docs) ;;
	*)
...
if [ "${#groups[@]}" -eq 0 ]; then
	groups=(static integration performance ai languages projects project_actions toolchain terminals workflow_actions signature lifecycle pdf_lifecycle windows layout_session ui_themes offline_assets deployment)
fi
